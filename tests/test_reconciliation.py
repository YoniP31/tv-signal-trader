...
class FakeOpenTradeCard:
    ...
    def __init__(self, company, portfolio):
        self.company = company
        self.portfolio = portfolio
        self.close_btn = FakeElement("✕ Close Trade")
...
def _driver_with_cards(cards, buttons=()):
    ...
    lookups with `cards` and TAG_NAME 'button' lookups with `buttons`
    (each a FakeElement whose .text is the button label)."""
    driver = MagicMock()
...
class ReconciliationTests(unittest.TestCase):
    ...
    def test_pending_result_tp_is_reported_normally(self):
        with patch.object(ms.tg, "list_all_candidates", return_value=[(self.company, self.portfolio)]), \
             ...
             patch.object(ms.tg, "has_open_trade_card") as has_card_mock, \
        ...
        has_card_mock.assert_not_called()  # short-circuited: no need to check the grid
