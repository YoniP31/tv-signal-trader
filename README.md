...
1. Finds the already-open TradingGenerator window, or opens one (`tradinggenerator.open_tab`), logging in automatically if the session's expired. By default (`config.HIDE_TRADINGGENERATOR_WINDOW = True`), opened as its own separate OS-level window (not a tab of the main browser window) and immediately hidden via the Win32 API (`browser.hide_window_by_title`, Windows only) — removed from the taskbar and Alt-Tab entirely rather than just minimized, so it's not reachable through normal user interaction, while Selenium keeps driving it normally (WebDriver commands talk to the browser over the DevTools protocol, not OS-level input/focus, so a hidden window responds to clicks/reads exactly like a visible one). Set that flag to `False` (a plain code edit, not `.env`-configurable) to open it as an ordinary visible tab instead, with no hiding — handy for debugging.
