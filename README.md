...
- **No-trade window** (`NO_TRADE_START_TIME`/`NO_TRADE_END_TIME`), **take-profit cap buffer** (`TP_CAP_BUFFER_MIN`/`TP_CAP_BUFFER_MAX`), **daily profit/loss limits** (`DAILY_PROFIT_LIMIT`/`DAILY_LOSS_LIMIT` — both **required**: a blank or missing one is flagged as an error and `web`/`web_multi`/`test` won't run until it's set) and their cap buffer (`DAILY_PNL_CAP_BUFFER_MIN`/`DAILY_PNL_CAP_BUFFER_MAX`), and **max positions per company** (`MPPC`, forced to 1 for `web` regardless of this setting) — see [docs/WEB_MULTI.md](docs/WEB_MULTI.md), which documents the shared trading loop behind both `web` and `web_multi`.
...
At a high level, every iteration: refreshes any already-tracked open position(s) first (reporting/freeing any that resolved), checks whether the next signal is eligible to open given the concurrency rules, and if so clicks **GENERATE NEW TRADE**, reads the resulting trade parameters, connects the right Tradovate account, checks balance and daily P&L limits (capping the take-profit/stop-loss if a win or loss would cross either), places the order, and tracks it going forward. TradingGenerator's own window is opened hidden by default (`config.HIDE_TRADINGGENERATOR_WINDOW = True`, Windows only) — typing `web`/`web_multi` at the `>` prompt asks whether to show it just for that run instead (Enter keeps whatever `config.py` says); set `HIDE_TRADINGGENERATOR_WINDOW` to `False` in code if you want visible to be the persistent default. A run started automatically from the command line (`tv-signal-trader.exe web_multi`, e.g. by the remote-deployment Scheduled Task after an update or reboot) has nobody to ask, so it uses `config.HIDE_TRADINGGENERATOR_WINDOW_WHEN_AUTO_STARTED` instead -- `False` (visible) by default, and kept through any in-app browser restart of that run. The user build always keeps it hidden. Hiding happens via `browser.hide_new_window`: it snapshots existing OS windows right before opening TradingGenerator's, then hides whichever new one appears as soon as it exists, rather than waiting for the page to load and set its title (`browser.hide_window_by_title`, used only as a fallback if that immediate hide somehow misses it) — so it no longer sits visibly on screen for a few seconds first. Either way, the prompt only has an effect the first time TradingGenerator's window/tab is actually created in a given browser session — once it's open, later prompts (e.g. running `web` then `web_multi` back to back) can't retroactively hide/show it.
...
Typing `add_accounts` at the `>` prompt (admin build only) logs into TradingGenerator if needed, asks which company (1-8, the same list as `config.PROP_FIRMS`) to add accounts to — reusing its company tab if one already exists rather than creating a duplicate — then reads account names to create as portfolios from `accounts_to_add.txt`, next to `.env` (one name per line; blank lines and lines starting with `#` are ignored; the command prints this exact path and format when it runs). Start from [docs/accounts_to_add.txt](docs/accounts_to_add.txt), a committed template with example account names and the format explained inline, the same way [docs/.env](docs/.env) works. `accounts_to_add.txt` is gitignored (only the `docs/` copy is tracked), so your own account list never ends up committed. Prompts once per run for LIVE or EVAL, applied to every name in that batch. Any name that already exists as a portfolio for the selected company is skipped rather than recreated. Runs under `humanize.fast_mode()` (see [tv_signal_trader/humanize.py](tv_signal_trader/humanize.py)) since it only touches TradingGenerator, not TradingView/Tradovate, so there's no anti-bot reason to pace it at human speed. Implementation: `_run_add_accounts` in [tv_signal_trader/cli.py](tv_signal_trader/cli.py), `add_company`/`add_portfolio` in [tv_signal_trader/tradinggenerator.py](tv_signal_trader/tradinggenerator.py).
